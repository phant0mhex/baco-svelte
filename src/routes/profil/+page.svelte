<script>
  ...
  let targetUserId = null; // Le profil qu'on regarde
  ...
  async function initProfile() {
    try {
      ...
      if (!user) return; // Redirection login gérée ailleurs normalement
      currentUser = user;
              ...
              {#if isMyProfile || isAdmin}
                <div class="pt-4 flex justify-end">
                  <button 
                    ...
                    disabled={isSaving}
                    class="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold shadow-sm transition-all flex items-center gap-2 disabled:opacity-50"
                  >
        ...
        <div class="space-y-8">
          ...
          <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            ...
            <div class="space-y-3 max-h-60 overflow-y-auto pr-1">
              {#if infractions.length === 0}
                <p class="text-sm text-gray-500 dark:text-gray-400 italic text-center py-4">Aucune infraction active.</p>
              {:else}
