<script>
  ...
  onMount(async () => {
    ...
    if (!hasPermission(currentUserProfile, ACTIONS.REPERTOIRE_READ)) {
  ...
  $: if (isAuthorized && selectedCategories) handleCategoryChange();
  $: if (isAuthorized && (selectedCategories || selectedZones || searchTerm || sortOrder)) loadContacts();
  ...
  function openModal(contact = null) {
    // SÉCURITÉ WRITE
    if (!hasPermission(currentUserProfile, ACTIONS.REPERTOIRE_WRITE)) return;
  ...
  async function handleSubmit() {
    // SÉCURITÉ WRITE
    if (!hasPermission(currentUserProfile, ACTIONS.REPERTOIRE_WRITE)) return toast.error("Action non autorisée.");
  ...
  async function deleteContact(id) {
    // SÉCURITÉ DELETE
    if (!hasPermission(currentUserProfile, ACTIONS.REPERTOIRE_DELETE)) return toast.error("Suppression non autorisée.");
    ...
    <header class="flex flex-col md:flex-row md:justify-between md:items-end gap-4 border-b border-white/5 pb-6" 
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="flex gap-2">
            ...
            {#if hasPermission(currentUserProfile, ACTIONS.REPERTOIRE_WRITE)}
           ...
           style="color: rgb(var(--color-primary)); background-color: rgba(var(--color-primary), 0.1); border-color: rgba(var(--color-primary), 0.2);">
            <Phone class="w-3 h-3" /> {formatPhone(contact.tel)}
                                        ...
                                        <div class="flex gap-1 opacity-0 group-hover/item:opacity-100 transition-opacity">
                                            ...
                                            {#if hasPermission(currentUserProfile, ACTIONS.REPERTOIRE_DELETE)}
