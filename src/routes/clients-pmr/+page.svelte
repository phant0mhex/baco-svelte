<script>
  ...
  onMount(() => {
    // Récupérer la vue préférée du localStorage
  ...
  function setView(mode) {
    viewMode = mode;
    localStorage.setItem('baco-client-view', mode);
  }
  ...
  function changePage(newPage) {
    if (newPage < 1 || newPage > Math.ceil(totalRows / ROWS_PER_PAGE)) return;
    currentPage = newPage;
    loadClients();
  }
  ...
  function openModal(client = null) {
    if (client) {
      ...
      editingClient = { id: null, prenom: "", nom: "", telephone: "", type: "", remarques: "" };
    }
    isModalOpen = true;
  }
  ...
  function getTypeBadgeClass(type) {
    ...
    if (['CRP', 'CRF'].includes(type)) return 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300 border-red-200 dark:border-red-800';
    if (type === 'CRE') return 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300 border-yellow-200 dark:border-yellow-800';
    return 'bg-blue-50 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300 border-blue-200 dark:border-blue-800';
  }
      ...
      <div class="flex items-center gap-3">
        ...
        <div class="flex bg-gray-100 dark:bg-gray-700/50 rounded-xl p-1 border border-gray-200 dark:border-gray-600">
          <button 
            ...
            class="p-2 rounded-lg transition-all {viewMode === 'grid' ? 'bg-white dark:bg-gray-600 shadow text-blue-600 dark:text-blue-300' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700'}"
            title="Vue Grille"
          >
            <LayoutGrid size={18} />
          </button>
          <button 
            ...
            class="p-2 rounded-lg transition-all {viewMode === 'list' ? 'bg-white dark:bg-gray-600 shadow text-blue-600 dark:text-blue-300' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700'}"
            title="Vue Liste"
          >
            <List size={18} />
          </button>
  ...
  <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8 max-w-lg mx-auto sm:mx-0">
      <div class="relative group">
        ...
        <input 
          ...
          class="block w-full pl-10 pr-3 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm"
        />
      </div>
    </div>
    ...
    {:else if clients.length === 0}
      <div class="text-center py-24 bg-white dark:bg-gray-800 rounded-3xl border-2 border-dashed border-gray-200 dark:border-gray-700">
        <Users size={48} class="mx-auto text-gray-300 dark:text-gray-600 mb-4" />
        <h3 class="text-lg font-bold text-gray-900 dark:text-white">Aucun client trouvé</h3>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Modifiez votre recherche ou ajoutez un nouveau client.</p>
      </div>
                ...
                {#each clients as client}
                  <tr class="group hover:bg-blue-50/50 dark:hover:bg-blue-900/10 transition-colors duration-150">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                      <div class="flex flex-col">
                        ...
                        {#if client.updated_at}
                          <span class="text-[10px] text-gray-400 dark:text-gray-500">Maj: {new Date(client.updated_at).toLocaleDateString()}</span>
                        {/if}
                      </div>
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap align-middle">
                      ...
                      {:else}
                        <span class="text-xs text-gray-300 dark:text-gray-600 italic px-2">—</span>
                      {/if}
                    </td>

                    <td class="px-6 py-4 whitespace-nowrap text-center align-middle">
                      {#if client.type}
                        <span class="inline-flex items-center justify-center px-2.5 py-1 text-[10px] font-extrabold rounded-full border shadow-sm w-12 {getTypeBadgeClass(client.type)}">
                          {client.type}
                        </span>
                      {:else}
                        <span class="text-gray-300 dark:text-gray-600 text-lg leading-none">—</span>
                      {/if}
                    </td>

                    <td class="px-6 py-4 align-middle">
                      {#if client.remarques}
                        <div class="relative group/tooltip max-w-xs">
                          <p class="text-sm text-gray-600 dark:text-gray-300 truncate" title={client.remarques}>
                            {client.remarques}
                          </p>
                        </div>
                      {:else}
                        <span class="text-xs text-gray-300 dark:text-gray-600 italic">Aucune remarque</span>
                      {/if}
                    </td>

                    <td class="px-6 py-4 whitespace-nowrap text-right align-middle">
                      <div class="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button on:click={() => openModal(client)} class="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-xl transition-all" title="Modifier">
                          <Pencil size={16} />
                        </button>
                        <button on:click={() => deleteClient(client.id, client.nom)} class="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-xl transition-all" title="Supprimer">
                          <Trash2 size={16} />
                        </button>
                      </div>
                    </td>
      ...
      <div class="relative w-full max-w-lg bg-white dark:bg-gray-800 rounded-[2rem] shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in duration-200 border border-gray-200 dark:border-gray-700">
        <div class="flex items-center justify-between px-8 py-6 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 z-10">
          <h2 class="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
            {#if editingClient.id} <PenTool size={20} class="text-blue-500"/> {/if}
            {editingClient.id ? 'Modifier' : 'Nouveau client'}
          </h2>
