<script>
  ...
  // --- STYLES "GLASS & NÉON" (Modifications) ---
  
  // 1. La Tuile de base (Conteneur Verre pour les boutons)
  ...
  // 2. L'effet Néon au survol (Cible l'icône SVG à l'intérieur)
  const neonHover = `
  ...
  // 5. Générateur de classe pour les liens principaux
  $: getLinkClass = (path) => {
    const isActive = $page.url.pathname.includes(path);
    // On combine la tuile de base + padding + état (actif/inactif) + effet néon
    return `${glassTileBase} px-4 py-2 gap-2 text-sm font-semibold ${isActive ? activeTile : `${inactiveTile} ${neonHover}`}`;
  };

  // 6. Style pour les boutons carrés (Recherche, Notif...)
  const iconBtnClass = `${glassTileBase} p-2.5 ${inactiveTile} ${neonHover}`;

  // 7. Dropdowns (Votre style original conservé + légers ajustements)
  ...
  <div class="px-6 py-2">
    ...
    <div class="flex justify-between items-center">
      ...
      <div class="hidden md:flex flex-row items-center gap-1 w-full justify-between ml-6">
        ...
        <div class="flex flex-row items-center gap-2"> <a href="/operationnel" class={getLinkClass('operationnel')}>
            ...
            <div class="relative">
                <button 
                    ...
                    class="{glassTileBase} px-4 py-2 gap-2 text-sm font-semibold 
                           {activeDropdown === 'pmr' || $page.url.pathname.includes('pmr') ? activeTile : `${inactiveTile} ${neonHover}`}"
                >
            ...
            <div class="relative">
                <button 
                    ...
                    class="{glassTileBase} px-4 py-2 gap-2 text-sm font-semibold 
                           {activeDropdown === 'repertoire' || $page.url.pathname.includes('repertoire') ? activeTile : `${inactiveTile} ${neonHover}`}"
                >
            ...
            <div class="relative">
                <button 
                    ...
                    class="{glassTileBase} px-4 py-2 gap-2 text-sm font-semibold 
                           {activeDropdown === 'data' || $page.url.pathname.includes('data') ? activeTile : `${inactiveTile} ${neonHover}`}"
                >
